...
const FloatingInput : React.FC<{label: string; type: string; name: string; id:string;}> = ({ label, type, name, id }) => {
  ...
  return (
    <div className='relative mt-4'>
      ...
      />
      <label 
        className={`absolute left-4 top-2 text-[1rem] text-muted-foreground bg-white px-1 transition-all duration-300 ease-in-out peer-focus:top-[-10px] peer-focus:text-[0.8rem] peer-focus:text-primary peer-valid:top-[-10px] peer-valid:text-[0.8rem] peer-valid:text-primary`}
        htmlFor={id}
      >
        {label}
      </label>
...
const Content = () => {
  return (
    <div className='m-[1.5rem] bg-white rounded-[4px]'>
